...
class LinkedInBot:
    ...
    def is_logged_in(self) -> bool:
        ...
        try:
            ...
            # If URL has /feed or is linkedin.com home, likely logged in
            if "/feed" in current_url or current_url.rstrip("/") == "https://www.linkedin.com":
                return True

            # Also check if login page is NOT showing
            if "/login" not in current_url and "/checkpoint" not in current_url:
                # Check for any sign of logged-in state
    ...
    def create_post(self, text: str) -> dict:
        ...
        """
        try:
            self._page.goto(FEED_URL, wait_until="domcontentloaded", timeout=15000)
            time.sleep(3)

            # Click "Start a post" button (NOT the Video/Photo buttons below it)
            start_post_btn = None
            try:
                start_post_btn = self._page.wait_for_selector(
            ...
            for sel in editor_selectors:
                try:
                    editor = self._page.wait_for_selector(sel, timeout=5000)
                    if editor:
                        break
                except Exception:
                    continue
            ...
            for sel in post_selectors:
                try:
                    post_btn = self._page.wait_for_selector(sel, timeout=5000)
    ...
    def comment_on_post(self, post_url: str, comment_text: str) -> dict:
        ...
        try:
            ...
            comment_btn = self._page.query_selector('button[aria-label*="Comment"]')
            if comment_btn:
                comment_btn.click()
                time.sleep(2)
    ...
    def like_post(self, post_url: str) -> dict:
        ...
        try:
            ...
            like_btn = self._page.query_selector('button[aria-label*="Like"]')
            if not like_btn:
    ...
    def get_my_posts(self, limit: int = 10) -> dict:
        ...
        try:
            ...
            me_btn = self._page.query_selector('img.global-nav__me-photo')
            if me_btn:
                me_btn.click()
                time.sleep(1)
                view_profile = self._page.query_selector('a[href*="/in/"]')
                if view_profile:
                    view_profile.click()
                    time.sleep(3)

            # Navigate to activity page
            activity_link = self._page.query_selector('a[href*="/recent-activity/"]')
            if activity_link:
                activity_link.click()
                time.sleep(3)

            posts = []
            post_elements = self._page.query_selector_all('div.feed-shared-update-v2')

            for el in post_elements[:limit]:
                try:
                    text_el = el.query_selector('span.break-words')
    ...
    try:
        ...
        if len(sys.argv) > 1:
            ...
            if action == "post" and len(sys.argv) > 2:
                result = bot.create_post(" ".join(sys.argv[2:]))
                print(json.dumps(result, indent=2))
            elif action == "comment" and len(sys.argv) > 3:
                result = bot.comment_on_post(sys.argv[2], " ".join(sys.argv[3:]))
                print(json.dumps(result, indent=2))
            elif action == "like" and len(sys.argv) > 2:
                result = bot.like_post(sys.argv[2])
                print(json.dumps(result, indent=2))
            elif action == "posts":
                result = bot.get_my_posts()
                print(json.dumps(result, indent=2))
            elif action == "login":
